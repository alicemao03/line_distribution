...
export function renderChord(members, data, svg, color, width, height, margin) {
    ...
    for (let i = 0; i < data.length - 1; i++) {
        ...
        var nextMembers = nextLine.member.filter(m => m.toLowerCase() !== "adlib" && m.toLowerCase() !== "all")

        while (nextMembers.length == 0) {
            i += 1
            nextLine = data[i + 1]
            nextMembers = nextLine.member.filter(m => m.toLowerCase() !== "adlib" && m.toLowerCase() !== "all")
        }

        console.log("currentMembers", currentMembers, nextMembers)
